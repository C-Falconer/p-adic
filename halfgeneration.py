...
print(colors.UNDERLINE + "n\tTag Half" + colors.END, flush=True, end="")
last_denom = 0
denoms = []
converted_halves = []
x, y = [], [0]
for i, item in enumerate(result):
    ...
    print(color(tag_half), end="\t", flush=True); continue
    print(color(tag_half), converted_halves[-1], end = "\t", flush=True)
    if denom == last_denom:
